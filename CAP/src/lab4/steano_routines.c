...
void imRGB2im(t_sRGB *imRGB, uint8_t *im, int *w, int *h)
{
	...
	*w = imRGB->w;
	*h = imRGB->h;
	// INtuiciojn mia: esto también se podra paralelizar
	for (int i=0; i<*h; i++)
		for (int j=0; j<*w; j++)
		{
...
void idct8x8_2d(float *in, float *out, int width, int height, float *mcosine, float *alpha)
{
	int bM=8;
	int bN=8;
	// INtuicion mia, aqui tambien se podra usar lo mismo que en la funcion anterior
	for(int bi=0; bi<height/bM; bi++)
	{
		int stride_i = bi * bM;
		for(int bj=0; bj<width/bN; bj++)
...
void encoder(char *file_in, char *file_out, char *msg, int msg_len)
{
	...
	// SI se puede paralelizar mejor. Pero la mejora no será muy significativa. ESto aplica a las dos funciones
